...
class DijkstraGrid:
    ...
    def build_map(self, obstacles, start_x=None, start_y=None):
        ...
        # Hard inflation must match the collision check radius (0.7m to ensure A* avoids heuristic traps)
        # Using exactly 0.7m so the Dijkstra heuristic is admissible
    ...
    def get_heuristic(self, wx, wy, wth=None):
        ...
        # Penalize severely if out of grid bounds, pushing it back towards the grid center
        euc = math.hypot(wx - self.goal_x, wy - self.goal_y)
    ...
    while q:
        ...
        for act, N, traj in precomp_prim:
            ...
            if prev_gear != 'N' and act[0] != prev_gear:
                ...
                # 防揉搓：追踪历史档位切换次数
                # 正常的 K-turn 会有 F->R->F 或 R->F->R（2次换挡，即1次揉搓）。
                # 用户要求：超过两次的“揉搓”要防。所以我们允许最多3次换挡（1.5次揉搓），第4次及以上（≥2次揉搓）严罚。
                gc_count = 0
                curr_node = path_node
                curr_gear = prev_gear
                lookback_depth = 0
            ...
            if visited.get(nkey, float('inf')) > new_cost:
                ...
                if _heuristic_preapproach:
                    ...
                    # 若角度偏大，说明还需要空间揉库，放宽 x 的上限到 max(_goal_xmax, 4.0)
                    dy_err = h_y / 5.0
                    dx_err = nx - _goal_xmin
                    kturn_est = 0.0
                    needed_x = 0.0
                    if (dy_err > 0.5 or abs_nth > _goal_thmax + 0.2) and dx_err > 0.0:
                        needed_x = dy_err * 2.0 + abs_nth * 1.5
                        if dx_err < needed_x:
                            kturn_est = (needed_x - dx_err) * 10.0
                            
                    dyn_xmax = max(_goal_xmax if abs_nth <= _goal_thmax + 0.2 else max(_goal_xmax, 4.0), _goal_xmin + needed_x)
                    ...
                    if dijkstra_grid is not None:
                        ...
                        # Always compute h_rs and use max() to avoid heuristic traps where 
                        # 2D h_grid underestimates parallel-parking or S-curve costs.
                        # Using max(h_rs, h_grid) ensures obstacle awareness from h_grid 
                        # and non-holonomic constraints from h_rs.
                        ...
                        # 补充非完整约束揉库预估惩罚（即使被障碍物阻挡，横向偏差大且纵向空间不足也必将揉库）
                        # 这避免了 h_grid 低估揉库代价导致 A* 陷入数万节点的探索泥潭
                        dy_err = abs(ny - RS_GOAL_Y)
                        dx_err = nx - max(RS_GOAL_X, 2.0) # 以托盘前方 2.0m 为对齐参考点
                        th_err = abs(nth - RS_GOAL_TH)
                        if (dy_err > 0.5 or th_err > 0.2) and dx_err > 0.0:
                            needed_x = dy_err * 2.0 + th_err * 1.5
                            if dx_err < needed_x:
                                # Multiplier 必须大到足以克服向外开导致的 cost 和 h_grid 增加！
                                # 向外开 1m 会导致 cost+4，h_grid*1.5*2.5+12 = 16 的增加。
                                # 所以每米需要补偿至少 16/2.5 = 6.4。设置 10.0。
                    ...
                    if nx < room_needed and (dy_h > 0.05 or dth_h > 0.05):
                        # 系数由 2.0→5.0（≈1/v_reverse），去除硬 cap，因为防揉搓惩罚很高
                        # 让启发式能预见揉库的高昂代价，避免 A* 陷入无休止的搜索
...
def _k_turn_preposition(x0, y0, theta0, precomp_prim, no_corridor=False, obstacles=None, dijkstra_grid=None, ignore_obs_for_y_range=False, target_y_max=None, x_ceil=None):
    """
    确定性 K-turn 预定位（三阶段）：
    ...
    不依赖 A*，总是在毫秒内完成。
    x_ceil: 可选的 x 上限，防止 K-turn 把车倒退过远（用于已穿过障碍物后不再倒回）。
    ...
    def _get_safe_y_range():
        y_min = -_eff_y_max
        y_max = _eff_y_max
        if obstacles and not ignore_obs_for_y_range:
            for obs in obstacles:
                min_x, max_x, min_y, max_y = obs
    ...
    def _score_y(ex, ey, eth, gear=None, prev_gear=None, w_x=1.0):
        ...
        # 即使 y 已在安全范围内，也鼓励向 y=0 靠拢。
        # Stage-2 目标 y ∈ [-0.18, 0.18]，在安全范围内越接近 0 越好。
...
def plan_path_robust(x0, y0, theta0, precomp_prim,
    ...
    # 障碍物场景下收紧 K-turn 目标 y：Stage-2 对齐目标仅 ±0.18，
    # 如果 K-turn 只把 y 收到 ±0.5，Stage-2 在障碍物限制下几乎无法完成最后 0.3m 的横向修正
    ...
    if phase0_acts or abs(my) > PREAPPROACH_Y_MAX or abs(mth) > 0.5 or (fast_obstacles is not None):
        ok1_greedy, acts1_greedy, mx2, my2, mth2 = _k_turn_preposition(
            mx, my, mth, precomp_prim, no_corridor=no_corridor, obstacles=fast_obstacles,
    ...
    # ── 阶段间对齐（Stage-1.5）：贪心 K-turn，专门处理 Stage-1 遗留的巨大 Y 偏差 ──
    t15_start = _time.perf_counter()
    stage15_acts = []
    # 计算 x_ceil：已穿过的障碍物的 min_x - 0.1，防止 K-turn 倒退回障碍物后方
    s15_x_ceil = None
    if fast_obstacles:
    ...
    t15_ms = round((_time.perf_counter() - t15_start) * 1000.0, 1)
    if stats is not None:
        stats['stage15_ms'] = t15_ms
    ...
    # Stage-2: 精准到位 — 解锁 X 向前空间，并且加入障碍物检测，赋予真正避障能力
    ...
    ok2, acts2, rs_traj = plan_path(
        ...
        obstacles=fast_obstacles, dijkstra_grid=dijkstra_grid
    )
    ...
    st_fallback2 = {}
    ok_fallback, acts_fallback, traj_fallback = plan_path(x0, y0, theta0, precomp_prim,
                                                          use_rs=use_rs, stats=st_fallback2,
                                                          no_corridor=no_corridor,
                                                          rs_expansion_radius=rs_expansion_radius,
