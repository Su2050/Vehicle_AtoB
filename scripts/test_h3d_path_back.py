...
print(f"Start: gx={gx}, gy={gy}, gth={gth}, h_3d={grid.dist_3d[gx][gy][gth][1]}")
    ...
    for d_step in [0.25, -0.25]:
        for steer in [0.0, 0.1, -0.1]:
            ...
            if 0 <= nx_g < grid.nx_3d and 0 <= ny_g < grid.ny_3d:
                ...
                idx = 0 if d_step > 0 else 1
                d = grid.dist_3d[nx_g][ny_g][nth_g][idx]
                if d < best_d:
                    best_d = d
                    best_next = (nx_g, ny_g, nth_g, d_step, steer, idx)
                    
    if best_next:
        curr_gx, curr_gy, curr_gth, d_step, steer, idx = best_next
        dir_str = "FWD" if idx == 0 else "BWD"
        print(f"Step {i+1}: gx={curr_gx}, gy={curr_gy}, gth={curr_gth}, d_step={d_step}, steer={steer}, dir={dir_str}, h_3d={best_d:.2f}")
    else:
        print("No path found")
        break
