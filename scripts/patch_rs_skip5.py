...
old_rs_check = """        # If dist_map is significantly larger than euclidean distance, it means there are obstacles
...
new_rs_check = """        # If h_3d is significantly larger than euclidean distance, it means there are obstacles
        # in the way or a complex maneuver is required. Skip RS expansion to save time.
        if dijkstra_grid is not None:
            h_3d = dijkstra_grid.get_3d_heuristic(cx, cy, cth)
            if h_3d > euclidean_goal * 1.5 + 1.0:
                return None"""
